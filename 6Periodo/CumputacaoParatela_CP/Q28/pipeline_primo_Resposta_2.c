...
void removeMultipleOf5() {  	
   ...
   while (1)  {
     ...
     if (number % 5 > 0) {
       // enviar para o próximo estágio para processos de 2 até N
		for(int i = 2; i < size; i++){
		MPI_Send(&number,1,MPI_INT,i,PIPE_MSG,MPI_COMM_WORLD);
		}
     }
   }
   // enviar mensagem de finalização -> Para todos os processos de 2 até N
    for (int i = 2; i < size; i++ ) {
